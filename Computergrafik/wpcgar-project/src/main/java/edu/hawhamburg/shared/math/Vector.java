...
public class Vector implements Serializable {
    ...
    public static Vector addDimension(Vector source) {
        return addDimension(source, 1);
    }

    public static Vector addDimension(Vector source, double value) {
        int sourceDimension = source.getDimension();
        Vector v = new Vector(sourceDimension + 1);

        for (int i = 0; i < sourceDimension; i++) {
            v.set(i, source.get(i));
        }
        v.set(sourceDimension, value);
        return v;
    }

    public static Vector removeDimension(Vector source) {
        int newDimension = source.getDimension() - 1;
        Vector v = new Vector(newDimension);
        for (int i = 0; i < newDimension; i++) {
            v.set(i, source.get(i) / source.get(newDimension));
        }
        return v;
    }
